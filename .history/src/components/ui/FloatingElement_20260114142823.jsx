...
export default function FloatingElement({
  ...
  // Skip animation for static preset
  if (preset === "static") {
    return (
  ...
  useEffect(() => {
    if (!floatTweenRef.current || !reduceOnHover) return;
  ...
  }, [isHovered, reduceOnHover]);

  // Mouse parallax effect
  useEffect(() => {
    if (!enableParallax || !elementRef.current) return;
    ...
    const handleMouseMove = (e) => {
      ...
      // Apply parallax with delay and easing
      const parallaxStrength = config.parallaxStrength * (1 + depth * 0.3);
      gsap.to(elementRef.current, {
        "--parallax-x": mousePos.current.x * 30 * parallaxStrength,
        "--parallax-y": mousePos.current.y * 20 * parallaxStrength,
        duration: 1.5,
        ease: "power2.out",
        overwrite: "auto",
      });
  ...
  }, [enableParallax, config.parallaxStrength, depth]);
