...
function Globe({ isHovered, images = [] }) {
  ...
  useEffect(() => {
    ...
    // Add glowing nodes at intersections (deterministic)
    ctx.fillStyle = ALADDIN_BLUE.primary;
    for (let i = 1; i < 12; i++) {
      for (let j = 0; j < 24; j++) {
        if (seededRandom(i * 100 + j) > 0.7) {
...
const FLOATING_PARTICLES = Array.from({ length: 30 }, (_, i) => ({
  id: i,
  width: (Math.sin(i * 1.5) * 0.5 + 0.5) * 4 + 2,
  height: (Math.cos(i * 1.7) * 0.5 + 0.5) * 4 + 2,
  left: (i * 37) % 100,
  top: (i * 53) % 100,
  opacity: (Math.sin(i * 2.3) * 0.5 + 0.5) * 0.5 + 0.2,
  duration: (Math.sin(i * 1.9) * 0.5 + 0.5) * 10 + 10,
  delay: (i * 0.7) % 10,
}));
