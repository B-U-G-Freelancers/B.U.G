...
function createTextTexture(
  ...
  // DEBUG: Draw background to verify plane visibility
...
class Title {
  constructor({
    ...
    this.textColor = textColor;
    this.font = font;
    // Don't auto-create mesh yet, wait for explicit call or create in constructor but don't parent
    this.createMesh();
  }
  ...
  createMesh() {
    ...
    // Initial scale - will be updated in update()
    this.mesh.scale.set(1, 1, 1);
    this.mesh.setParent(this.plane.parent); // Parent to scene, same as plane
  }

  update(planePosition, planeRotation, planeScale) {
    if (!this.mesh) return;

    // Copy parent plane transforms
    this.mesh.position.copy(planePosition);
    this.mesh.rotation.copy(planeRotation);

    // Apply local offset logic here
    const textHeight = planeScale.y * 0.12;
    // We can't easily get the aspect ratio here without storing it, so let's store it
    if (!this.aspect) this.aspect = this.mesh.scale.x / this.mesh.scale.y || 5;

    const textWidth = textHeight * this.aspect;
    this.mesh.scale.set(textWidth, textHeight, 1);

    // Move Y relative to the rotated/positioned plane?
    // Actually simplicity: Since we copied rotation, 'up' is local Y.
    // We want to move 'up' in local space. OGL meshes don't have 'translateY' easily?
    // Let's use simple trigonometry or just keep it as child BUT disable the parent's shader effects on children?
    // OGL custom shaders don't affect children unless the children share the program.
    // So the previous issue was likely depth or just sizing.
    // But let's try the detached approach for total control.

    // Actually, detaching and syncing rotation/position manually is complex with rotations.
    // Let's instead keep it attached but fix the Z and scale logic.
    // The previous fix MIGHT have worked if the texture wasn't empty.
    // Let's go with the DEBUG TEXTURE approach first + Child.
  }
}
