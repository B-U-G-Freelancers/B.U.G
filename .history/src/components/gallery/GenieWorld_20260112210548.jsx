...
function ProjectCarousel({
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      scrollTargetRef.current = Math.max(
        0,
        Math.min(maxScroll, scrollTargetRef.current)
      );
    };
  ...
  const handleMouseUp = () => {
    ...
    const nearestIndex = Math.round(
      scrollTargetRef.current / (cardWidth + gap)
    );
    const clampedIndex = Math.max(
      0,
      Math.min(projects.length - 1, nearestIndex)
    );
        ...
        {projects.map((project, index) => (
          <div
            ...
            style={{
              ...
              transform: `translateX(${index * (cardWidth + gap)}px) scale(${
