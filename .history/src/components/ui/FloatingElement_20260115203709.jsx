...
import { FLOAT_PRESETS } from "./floatPresets";
...
// Floating background fragments for atmospheric depth - OPTIMIZED
export function FloatingFragments({ count = 8 }) {
  // Generate only orbs (most impactful, least heavy)
  const orbs = Array.from({ length: Math.min(count, 8) }, (_, i) => {
    const seed = i + 1;
    const size = seededRandom(seed, 20, 60);
    const top = seededRandom(seed * 2, 10, 90);
    const left = seededRandom(seed * 3, 10, 90);
    const opacity = seededRandom(seed * 4, 0.12, 0.25);

    return { id: i, size, top, left, opacity };
  });

  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      {/* Radial gradient overlay for depth - static, no animation */}
      <div
        className="absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse at 30% 20%, rgba(59, 130, 246, 0.12) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(99, 102, 241, 0.08) 0%, transparent 40%)",
        }}
      />
      ...
      {/* Floating orbs - CSS animations instead of JS for better performance */}
      {orbs.map((orb) => (
        <div
          key={orb.id}
          ...
          style={{
            ...
            width: orb.size,
            height: orb.size,
            backgroundColor: `rgba(59, 130, 246, ${orb.opacity})`,
            filter: `blur(${orb.size / 4}px)`,
            animation: `float-${orb.id % 3} ${
              8 + orb.id * 2
      ...
      {/* CSS keyframes for floating - GPU accelerated */}
      <style>{`
        @keyframes float-0 {
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(58, 124, 255, 0.4) 0%, transparent 60%)",
        }}
      />
      <div
        className="absolute bottom-0 right-0 w-56 h-56 opacity-[0.025]"
        style={{
          background:
            "linear-gradient(-45deg, rgba(58, 124, 255, 0.4) 0%, transparent 60%)",
        }}
      />
    </div>
  );
