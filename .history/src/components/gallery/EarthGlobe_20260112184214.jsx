...
// Generate initial particle data outside of component
function createInitialParticleData(particleCount) {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    lifetimes[i] = 0;
    sizes[i] = seededRandom(i * 7) * 0.08 + 0.02;
  }
...
function GlitterParticles({ active, mousePos }) {
  ...
  // Use useMemo for initial data to avoid creating during each render
  const particleData = useMemo(
    () => createInitialParticleData(particleCount),
    [particleCount]
  );
  ...
  useFrame(() => {
    ...
    const { positions, velocities, lifetimes, sizes } = dataRef.current;
...
function NeptunePlanet({ isHovered }) {
  ...
  // Load textures (only using main diffuse and bump for now)
  const [diffuseMap1, , bumpMap1] = useTexture([
...
// Main component
export default function EarthGlobe({ onEnter }) {
