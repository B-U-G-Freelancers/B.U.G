...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Desaturate effect (grayscale) - reduced on hover
          float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));
          color.rgb = mix(vec3(gray), color.rgb, uHover);
  ...
  update(scroll, direction) {
    ...
    const scale = baseScale + (hoverScale - baseScale) * this.hoverProgress;
  ...
  onMouseMove(e) {
    ...
    if (this.medias) {
      this.medias.forEach((media, i) => {
        const screenX = media.plane.position.x / (this.viewport.width / 2);
        const screenY = media.plane.position.y / (this.viewport.height / 2);
    ...
    if (closestIndex !== this.hoveredIndex) {
      ...
      if (this.medias) {
        this.medias.forEach((media, i) => {
          media.setHover(i === closestIndex);
        });
      }
      this.hoveredIndex = closestIndex;
      if (this.onItemHover && closestIndex >= 0) {
        this.onItemHover(this.medias[closestIndex].projectData, closestIndex);
      }
    }
  }
  onClick(e) {
    if (this.hoveredIndex >= 0 && this.onItemClick && this.medias) {
      const media = this.medias[this.hoveredIndex];
      this.onItemClick(media.projectData, this.hoveredIndex);
    }
  }
  onResize() {
    this.screen = {
