...
function ProjectCarousel({
  ...
  const smoothScrollX = useSpring(scrollX, { damping: 30, stiffness: 100 });
  ...
  useEffect(() => {
    scrollX.set(-currentIndex * (cardWidth + gap));
  ...
  const handleDrag = (_, info) => {
    ...
    const clampedIndex = Math.max(
      0,
      Math.min(projects.length - 1, currentIndex + newIndex)
    );
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" ref={containerRef}>
      <motion.div
        className="flex items-center h-full px-[calc(50vw-175px)]"
        style={{ x: smoothScrollX, gap: `${gap}px` }}
        drag="x"
      ...
      <button
        ...
        className="absolute left-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all"
      ...
      <button
        ...
        className="absolute right-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all rotate-180"
