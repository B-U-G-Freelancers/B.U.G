...
export function FloatingFragments({ count = 8 }) {
  ...
  const orbs = Array.from({ length: Math.min(count, 8) }, (_, i) => {
    const seed = i + 1;
    const size = seededRandom(seed, 8, 28);
    const top = seededRandom(seed * 2, 10, 90);
    const left = seededRandom(seed * 3, 10, 90);
    const opacity = seededRandom(seed * 4, 0.05, 0.15);
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 30% 20%, rgba(58, 124, 255, 0.04) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(58, 124, 255, 0.03) 0%, transparent 40%)",
        }}
      />
      ...
      {orbs.map((orb) => (
        <div
          ...
          style={{
            ...
            width: orb.size,
            height: orb.size,
            backgroundColor: `rgba(58, 124, 255, ${orb.opacity})`,
