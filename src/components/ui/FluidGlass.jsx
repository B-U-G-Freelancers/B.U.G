...
export default function FluidGlass({
  ...
  images = [
    ...
    { url: "/demo/demo3.jpg", position: [-2.05, -6, 6], scale: [1, 3, 1] },
  ],
}) {
...
function Lens({ modeProps, backgroundColor, ...p }) {
  return (
    <ModeWrapper
      glb="/3d/lens.glb"
...
function Cube({ modeProps, backgroundColor, ...p }) {
  return (
    <ModeWrapper
      glb="/3d/cube.glb"
      geometryKey="Cube"
...
function Bar({ modeProps = {}, backgroundColor, ...p }) {
  const defaultMat = {
    transmission: 1,
    roughness: 0,
    thickness: 10,
...
function Images({ images }) {
  ...
  useFrame(() => {
    ...
    group.current.children.forEach((child, i) => {
      if (child.material) {
        const range =
          i < 2 ? data.range(0, 1 / 3) / 3 : data.range(1.15 / 3, 1 / 3) / 2;
        child.material.zoom = 1 + range;
      }
    });
  });
