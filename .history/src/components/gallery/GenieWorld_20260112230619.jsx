...
function ProjectCarousel({
  ...
  return (
    ...
    >
      {/* Circular carousel - cards positioned on arc */}
      <div
      ...
      >
        {projects.map((project, index) => {
          ...
          const radius = 800; // Arc radius
          const anglePerCard = 15; // Degrees between cards
          const angle = offset * anglePerCard;
          const radians = (angle * Math.PI) / 180;

          // Calculate position on arc
          const x = Math.sin(radians) * radius;
          const z = Math.cos(radians) * radius - radius; // Push center card forward
          ...
          const opacity = isFocused
            ? 1
            : Math.max(0.3, 1 - Math.abs(offset) * 0.25);
