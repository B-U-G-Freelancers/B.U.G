...
// Carousel of tilted cards with smooth motion-based scrolling
function ProjectCarousel({
  ...
  const cardWidth = 450; // Bigger cards
  const gap = 60;
  const ease = 0.08;
  ...
  useEffect(() => {
    const animate = () => {
      ...
      // Update cards position via transform
      const container = containerRef.current;
      if (container) {
        const cards = container.querySelectorAll('.carousel-card');
        cards.forEach((card, i) => {
          const x = i * (cardWidth + gap) - scrollCurrentRef.current;
          const centerOffset = window.innerWidth / 2 - cardWidth / 2;
          card.style.transform = `translateX(${x + centerOffset}px)`;
          
          // Determine if this is the center card
          const cardCenter = x + centerOffset + cardWidth / 2;
          const screenCenter = window.innerWidth / 2;
          const distance = Math.abs(cardCenter - screenCenter);
          const isCenter = distance < cardWidth / 2;
          
          // Update current index based on center card
  ...
  // Sync scroll target with current index
  useEffect(() => {
    scrollTargetRef.current = currentIndex * (cardWidth + gap);
  }, [currentIndex]);
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      scrollTargetRef.current += delta * 0.5;
      
      // Clamp scroll target
      const maxScroll = (projects.length - 1) * (cardWidth + gap);
  ...
  const handleMouseDown = (e) => {
    ...
    dragScrollRef.current = scrollTargetRef.current;
  };

  const handleMouseMove = (e) => {
    if (!isDraggingRef.current) return;
    const x = e.clientX || e.touches?.[0]?.clientX || 0;
    const delta = (dragStartRef.current - x) * 1.5;
    scrollTargetRef.current = dragScrollRef.current + delta;
  ...
  const handleMouseUp = () => {
    ...
    const nearestIndex = Math.round(scrollTargetRef.current / (cardWidth + gap));
    const clampedIndex = Math.max(0, Math.min(projects.length - 1, nearestIndex));
    scrollTargetRef.current = clampedIndex * (cardWidth + gap);
    ...
    >
      ...
      <div className="relative w-full h-full flex items-center">
        {projects.map((project, index) => (
          <div
            key={project.id}
            className="carousel-card absolute left-0 top-1/2 -translate-y-1/2 transition-transform duration-100"
            style={{ 
              opacity: index === currentIndex ? 1 : 0.4,
              transform: `translateX(${index * (cardWidth + gap)}px) scale(${index === currentIndex ? 1 : 0.8})`,
      ...
      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 text-white/40 text-xs tracking-widest uppercase z-20">
        Scroll, drag, or use arrows to navigate
      </div>
    </div>
  );
}
      ...
      <button
        onClick={() => setCurrentIndex(Math.max(0, currentIndex - 1))}
        className="absolute left-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all disabled:opacity-30"
        disabled={currentIndex === 0}
      >
      ...
      <button
        ...
        className="absolute right-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all rotate-180 disabled:opacity-30"
        disabled={currentIndex === projects.length - 1}
      >
        <ArrowLeft className="w-5 h-5" />
      </button>
      ...
      {/* Scroll hint */}
      <div className="absolute bottom-20 left-1/2 -translate-x-1/2 text-white/30 text-xs tracking-widest uppercase">
      ...
      <AnimatePresence>
        {currentState === STATES.GALLERY && (
          ...
          >
            ...
            <div className="absolute top-1/2 left-8 -translate-y-1/2 max-w-xs pointer-events-none">
              <motion.div
                key={currentIndex}
